...
- persisted time series use positional bucket layouts with rounded values; the power monitor keeps 96 quarter-hour aggregate buckets for its 24-hour figures plus its chart tiers, roughly 30 KB in practice and under 64 KB at its theoretical widest (`src/test/storage/footprint.lua` writes that worst case through the real store and asserts the budget); the dashboard's incident history stays under 24 KB;
