...
All history goes through `lib/timeline.lua`. An application commits one record per sample at whatever cadence suits it — a number, or a table of named numeric fields — and the timeline downsamples it into fixed-step buckets for each window (15 minutes at the sample rate up to 24 hours at 15-minute steps). Each field declares which aggregates it keeps (`last`, `sum`, `min`, `max`) and its persisted precision, so a chart series keeps everything while a rate that is only ever averaged keeps a sum. `values()` feeds charts, `aggregate()` answers "over the last N seconds" questions, and `export()`/`restore()` are the only persistence format. The power monitor and the line monitor share this one implementation; neither keeps a private bucket store.
...
- persisted time series use positional bucket layouts with rounded values; the power monitor's five-field timeline is roughly 30 KB in practice and under 64 KB at its theoretical widest (`src/test/storage/footprint.lua` writes that worst case through the real store and asserts the budget); the dashboard's incident history stays under 24 KB;
